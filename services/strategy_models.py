...
class SMAModel(FinancialData):
    ...
    def prepare_indicators(self, window: None, short: int, long: int) -> None:
        self.data['Short SMA'] = self.data['Adj Close'].rolling(short).mean()
        self.data['Long SMA'] = self.data['Adj Close'].rolling(long).mean()
    ...
    def prepare_signals(self, window: None, short: None, long: int) -> None:
        ...
        for day in range(len(self.data)):
            if day <= long:
                signal.append(0)
            elif self.data['Short SMA'].iloc[day-2] < self.data['Long SMA'].iloc[day-2]\
                and self.data['Short SMA'].iloc[day-1] > self.data['Long SMA'].iloc[day-1]:
                    signal.append(1)
            elif self.data['Short SMA'].iloc[day-2] > self.data['Long SMA'].iloc[day-2]\
                and self.data['Short SMA'].iloc[day-1] < self.data['Long SMA'].iloc[day-1]:
                    signal.append(-1)
...
class MACDModel(FinancialData):
    ...
    def prepare_indicators(self, window: None, short: int, long: int) -> None:
        self.data['Short EMA'] = self.data['Adj Close'].ewm(span=short, adjust=False).mean()
        self.data['Long EMA'] = self.data['Adj Close'].ewm(span=long, adjust=False).mean()
        self.data['DIFF'] = self.data['Short EMA'] - self.data['Long EMA']
        self.data['DEM'] = self.data['DIFF'].ewm(span=9, adjust=False).mean()
